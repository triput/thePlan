...
| **P1.5** | Real login story: password session and/or Cloudflare Access in front of Tunnel. Populate `password_hash`. Enable remote hosting without schema rewrite. |
...
- **Reserve remote path:** Home server + Cloudflare Tunnel (P1.5+) requires auth before exposure to internet. Schema and API paths designed now (`/auth/login`, `/auth/me`) avoid rewrite later.
...
4. OAuth tokens for **calendar** providers stored separately in `calendar_accounts` (P2), not user login.

## Security Notes

- MVP binds API to localhost/LAN; document that LAN exposure without P1.5 auth is dev-only.
...
- P1.5 adds auth stories to [06-mvp-backlog.md](../06-mvp-backlog.md) successor and [05-api-contract.md](../05-api-contract.md) auth section.
- Cloudflare Tunnel template deferred to P1.5 docs, not MVP Wave 4 blocker.
