...
**Forward (W2c):** True session undelete via task soft-delete (`deleted_at` on `tasks`; see [04-data-schema.md](./04-data-schema.md) and [07-wave-roadmap.md](./07-wave-roadmap.md)). Delete undo becomes restore API; client stack still handles complete/uncomplete.
...
| True soft-delete / session undelete | W2c |
