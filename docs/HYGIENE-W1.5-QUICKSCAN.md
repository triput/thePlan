...
| Local API venv | Python **3.11.9** | Acceptable (`requires-python >=3.11`); prefer 3.12 locally when convenient |
...
1. Align local venv to Python 3.12 when easy (Docker already 3.12).
2. Consider Nginx `/api` reverse-proxy before Cloudflare Tunnel (LAN cookie/CORS hygiene).
